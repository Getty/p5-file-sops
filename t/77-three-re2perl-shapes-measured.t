...
# /tmp/sops -- without it the Perl-side measurements still pin the limit
# IN THIS DISTRIBUTION, and SOPS_BIN that names a non-executable file is
# still a hard failure.

SKIP: {
    my $sops_bin = find_sops();
    skip "No sops binary found (checked \$SOPS_BIN, PATH, /tmp/sops) -- what "
       . "sops classifies these keys as was NOT measured, so the three "
       . "shapes above are pinned against nothing on the sops side. Run "
       . "maint/fetch-sops or set SOPS_BIN.", 4
        unless $sops_bin;
