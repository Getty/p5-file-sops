...
- `_compute_mac` calls `assert_representable` per leaf with `encrypted => 1`
  ...
  karr #59 message, naming the form (`+Inf` / `-Inf` / `NaN`) and pointing
  at type:str.
...
|---|---|---|
| `dualvar(+Inf, '.inf')` | `v: .inf` | `+Inf` (karr #113 / ADR 0031's row) |
| `dualvar(-Inf, '-.inf')` | `v: -.inf` | `-Inf` |
| `dualvar(NaN, '.nan')` | `v: .nan` | `NaN` |
| bare `+Inf` (no PV) | `v: .inf` | `+Inf` (karr #134 / ADR 0037's repair) |
...
JSON carrier croaks — measured, that is the existing karr #134 behaviour
...
#59 message the encrypt side already used.**

### 1. One predicate, one place

`assert_representable` already gates on `!_carries_go_non_finite_token` for
both branches (`encrypted => 0` and `=> 1`), with the karr #59 message that
...
(karr #134): the carrier manufactures a `dualvar($double, $token)` for the
...
*before* it; for a contradiction, the karr #59 message is the one that
...
The `assert_representable` body itself is unchanged. The karr #59 message
...
slot-appropriate `encrypted` value (karr #122 / ADR 0040). The new walk
...
- **`decrypt_file`, `edit`, and `_serialize_plaintext` refuse a contradicting
  ...
  karr #59 message naming the leaf's path; no file is left behind; no
...
- **No parse and no decryption produces a contradicting dualvar.** The
  reachable set is callers who construct one by hand — karr #140's
  ...
  high.
- **The croak fires with the karr #59 message and not a path-prefixed
...
- **JSON's bare-NV refusal is unchanged.** `_non_finite_token_leaf` still
  croaks with the karr #134 message for a bare `+Inf` in JSON — the
...
**Repair the leaf by dropping the string half.** The carrier already does
  this for a bare NV (karr #134), where there is no half to drop. A leaf
...
**Move the karr #59 message into the walk and call the consult from the
  ...
  signature defect the karr #113 / 0031 line of work set out to prevent.

**Tighten the gate to refuse the karr #113 / ADR 0031 row too.** A leaf
  ...
  correctly, which is the same defect karr #122 / ADR 0040 closed for
  the encrypted slot.
...
the karr #59 message, every refusing row in JSON is the same, the key
