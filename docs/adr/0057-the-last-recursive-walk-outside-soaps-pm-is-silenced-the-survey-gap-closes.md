...
- Date: 2026-08-23
- Resolves karr #179
- Closes karr #120's open question (ADR 0029's `## Consequences` last bullet)
...
`Format/YAML.pm` were held by karr #116 / #120 and could not be touched then:
...
remaining noise as the open work, and karr #120 promised to remove it.

Karr #120 silenced TWO walks: `Encrypted::_canonical_floats` and
`Format::YAML::_restring_non_finite_leaves`. Measured on t/45's 265-level
alias chain (the fixture from karr #117): 505 warnings before either change,
...
added to the parser at karr #127 (ADR 0054) after karr #120's survey. It is
...
fixture, after karr #120:

```
# 167 deep-recursion warnings left, from File::SOPS::Format::YAML::_go_repair_int_leaves -- karr #179
```

That was t/45's `What is still noisy` diag, reporting the residue exactly
because karr #179 was filed and open. The fix is the same shape karr #120
...
The fix is exactly the diff karr #120 applied to `_restring_non_finite_leaves`,
...
optimisation karr #120 applied to `_canonical_floats` is not in scope.
...
- **The karr #120 survey is now honest.** The ticket listed two walks and
  ...
  open bullet closes here, and karr #120's promise is kept.
...
- **No AAD or MAC moves.** The walk does not build a path; it reads each
  ...
  by `_sorted_leaves`/`_document_leaves`, which the karr #117 set silenced,
...
File::SOPS::Format::YAML::_go_repair_int_leaves -- karr #179`. After: the
